...
#(parameter WIDTH = 0, HSIZE = 0, HFP = 0, HSP = 0, HMAX = 0, VSIZE = 0, VFP = 0, VSP = 0, VMAX = 0, HSPP = 0, VSPP = 0)
    ...
    input reg[11:0] blueball_xc,
    input reg[11:0] blueball_yc,
    input reg [11:0] redball_xc,
    input reg[11:0] redball_yc,
    input wire[3:0] bg_stage,
    ...
    reg write;                    //正在写入的显�??????????????
    reg finish;                   //写入完成的标�??????????????
    reg [11:0] write_h;           //写横坐标
    reg [11:0] write_v;           //写纵坐标
    reg [11:0] read_h;            //读横坐标
    reg [11:0] read_v;            //读纵坐标
...
module back_pixel_gen(
    ...
    assign back_pixel = {rgb[23:16],rgb[7:0],rgb[15:8]};
endmodule
